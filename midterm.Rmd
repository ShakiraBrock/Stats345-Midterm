...
bucks_shots_24 <- bucks_shots_24_unclean %>% select("isShotMade", "locationX", "locationY", "yearSeason", "namePlayer")
...
bucks_shots_19 <- bucks_shots_19_unclean %>% select("isShotMade", "locationX", "locationY", "yearSeason", "namePlayer")
...
bucks_shots_14 <- bucks_shots_14_unclean %>% select("isShotMade", "locationX", "locationY", "yearSeason", "namePlayer")
...
bucks_shots_09 <- bucks_shots_09_unclean %>% select("isShotMade", "locationX", "locationY", "yearSeason", "namePlayer")


bucks_shots_list <- list(bucks_shots_24, bucks_shots_19, bucks_shots_14, bucks_shots_09)

bucks_shots_all <- reduce(bucks_shots_list, full_join, by = colnames(bucks_shots_24))
...
bball_court + 
geom_point(data = bucks_shots_all, size = .3, alpha = .2, aes(locationX/10, locationY/10, color = isShotMade)) + 
  ...
  labs(
    ...
    y = "Y Location",
    color = "Shot Outcome"
  ) + scale_x_reverse() + facet_wrap(~ yearSeason) + guides(color = guide_legend(override.aes = list(size = 4, alpha = 1))) + dark_theme_classic() +  theme(axis.line = element_blank(),
...
salaries <- hoopshype_salaries(
  ...
  filter(urlTeamSalaryHoopsHype == "https://hoopshype.com/salaries/milwaukee_bucks/")

salaries_clean <- salaries %>% filter(slugSeason == "2024-25") %>%
  select(namePlayer, amountContract) %>%
  mutate(
  ...
  select(-amountContract)

bucks_salaries <- bucks_shots_24 %>%
  left_join(salaries_clean, by = "namePlayer") %>% filter(!is.na(salary_bracket)) %>% mutate(salary_bracket = factor(salary_bracket, levels = c("High", "Middle", "Low")))

bball_court + 
  geom_point(data = bucks_salaries, 
             aes(locationX/10, locationY/10, color = isShotMade), 
             size = 0.5, alpha = 0.5) + 
  scale_colour_manual(values = c( "#8B0000", "#006400"), labels = c("Missed Shots", "Made Shots")) +
  labs(
    title = "Past Shoting Effectiveness and Future Contracted Pay",
    x = "X Location",
    y = "Y Location",
    color = "Shot Outcome"
  ) + 
...
```{r}
 bad_plot <- ggplot() + geom_point(data = bucks_shots_21, 
             aes(locationX, locationY, color = isShotMade), 
             size = 1, alpha = 0.5) + 
  scale_colour_manual(values = c( "#8B0000", "#006400"), labels = c("Missed Shots", "Made Shots")) +
  labs(
    title = "Past Shoting Effectiveness and Future Contracted Pay",
